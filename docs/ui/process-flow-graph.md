...
目前實作可以把這兩個概念對應到較短的 component prop，例如 `edit` 與 `view`。
...
Edge label 平常保持簡潔，但目前實作會常駐顯示一個短 slot label pill，例如
...
Graph UI 提供共用的 edge-level geometry preview button style 與位置。

上層 editor 決定某條 edge 是否顯示 preview button，因為 preview availability 取決於目前資料狀態：

- `stepOutput` edge 可以在 upstream step output data 足夠完整時 preview upstream step output。
- `geometryRef` edge 可以在上層 editor 已有具體 geometry id 時 preview selected geometry。
- 若 preview 不可用，上層 editor 可以隱藏 button，或顯示 disabled button 並提供說明 title。
...
本節記錄目前 `apps/viewer` 前端在 Browser 中觀察到的實際 UI 與互動。
它不是新的產品概念，而是為了未來重新實作時，可以照著還原同一個操作體驗。
...
目前 UI 使用 Next + Tailwind + shadcn-style primitives。復刻時應先建立同樣的
視覺語彙，再做 Graph。
...
| `minZoom` | `0.35` | 兩個 editor 都使用。 |
| `maxZoom` | `1.4` / instance `1.45` | Template editor 使用 1.4，Instance editor 使用 1.45。 |
| `fitView` | template true，instance 選 template 後手動 fit | Template editor 初次顯示自動 fit；instance 選 template 後 `fitView({ padding: 0.18, duration: 220 })`。 |
...
- 若沒有 selected node 但已有 nodes，新 step 放在目前最右 node 的 `x + 330`、
  `y = 160`，並立即打開 dialog。
...
- Graph 自動 fit view，padding `0.18`、duration `220ms`。
