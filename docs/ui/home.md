...
`processStepTemplates` currently seeds built-in templates backed by real modules
under `src/process/`: `molding` (`layer/molding`) and `PnP` (`pnp/pnp`).
Additional product process step templates can still be authored in Process Step
Editor.
