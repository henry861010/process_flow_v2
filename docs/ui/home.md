...
|---|---|---|---|
| `processStepTemplates` | `ProcessStepTemplate[]` | `[]` | Home initializes; admin editor reads/writes. |
...
`processStepTemplates` currently seeds as an empty array. Product process step templates should be authored in Process Step Editor and backed by real modules under `src/process/`.
