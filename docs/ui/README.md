...
| 文件 | 使用時機 |
| --- | --- |
| [Reconstruction Guide](reconstruction-guide.md) | 用 canonical fixture 重建 route、state 與主要操作旅程。 |
| [Review Checklist](review-checklist.md) | project implementation review、browser 檢閱與 evidence 紀錄。 |
...
| [Evidence](evidence/README.md) | current observation 與 accepted reference 的分層規則。 |
...
| `HbmGeneratorDialog` | [HBM Geometry Generator](components/hbm-generator.md) |

## 三種使用模式

| 使用者 | 先讀 | 目的 |
| --- | --- | --- |
| Reconstruction agent | [Reconstruction Guide](reconstruction-guide.md) → route/component spec | 不讀既有 code 也能以 fixtures 重建一致 UI。 |
| Project reviewer | [Review Checklist](review-checklist.md) → [conformance](../conformance.md) | 按 route、viewport、interaction 與 accessibility 逐項檢閱。 |
| Observer / designer | screen spec → `assets/reference` → `evidence` | 先理解 accepted target，再辨識 current implementation drift。 |
...
| `/flow-instance-editor` | 選 `CoWoS-L Demo`，不儲存 workspace | bind、edit、preview；確認draft controls/status box隱藏；known workspace URL commit | `UI-FIE-001` 至 `UI-FIE-008` |
