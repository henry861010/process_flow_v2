...
source_of_truth:
  - docs/architecture/decisions/0005-estimated-feature-visualization.md
  ...
  - apps/viewer/components/geometry-preview/geometry-feature-section-caps.tsx
...
Estimated feature section是獨立同步layer：section drag時立即依current plane與feature envelope更新，不等待
OCC；exact body仍遵守上述Computing/ready狀態。3D cap與2D SVG使用同一份feature section model。只要plane
與enabled feature envelope有positive-area intersection，就顯示固定world-anchored pattern，不要求切到
estimated glyph row。Section UI標示`Bodies: Exact`與`Features: Estimated`；feature-only section不得因
exact regions empty而消失。
...
Auto初始使用2,000 tier，依interaction frame time與hysteresis降至500或升至10,000；Summary不建立detail
marks。Cross-section pattern不使用此budget。
...
| Estimated instanced glyphs | density cue；low-poly bump/via/circuit surface proxy |
| Estimated feature section | envelope/plane交集的固定pattern；不是physical placement或exact material |

Colors：Via `#12a8c6`、Circuit `#2ea85d`、Bump `#d8ad2f`。Summary只render envelope+hatch；
Detail使用manual hard cap；Auto使用bounded quality tier。Total glyphs不得超過global cap，positive-density
visible features先取得presence allocation，剩餘budget按requested count deterministic分配。Density 0只
保留outline/metadata，不產生repeated marks。
...
Feature view開啟時，enabled feature bounds會merge進navigation、camera、grid、section slider與guide
bounds；exact section request可在body外合法回empty。Exact cap clipping epsilon仍只使用materialized
model bounds。
...
| `UI-PREVIEW-005` | Auto estimate超cap | 公平downsample到quality tier；所有positive-density feature保留presence機會。 |
...
| `UI-PREVIEW-012` | density feature visible | UI明示Estimated；feature不進exact response，但可在composite section以estimated pattern顯示。 |
...
| `UI-PREVIEW-017` | section plane inside enabled feature envelope | 3D cap與2D使用相同feature region/pattern，即使沒有glyph center仍顯示。 |
| `UI-PREVIEW-018` | exact body empty/loading且feature non-empty | Estimated feature section立即顯示，Exact狀態獨立。 |
| `UI-PREVIEW-019` | full view embedded/partially exposed feature | embedded由body depth隱藏；partial exposure與die gap feature可見。 |
