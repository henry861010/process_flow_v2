...
| `apps/viewer/app/api/geometry-preview/route.ts` | Server-side kernel execution 與 GLB export |
...
| Footer actions | 右下角 `Save JSON` 與 `Save GLB` buttons |
...
如果 validation、kernel execution 或 GLB export 失敗：

- Preview panel 保持開啟。
- 錯誤訊息顯示在 preview panel 裡。
- 不修改 graph draft。
- 不寫入 localStorage。
- `Save JSON` 與 `Save GLB` disabled。
