...
| UI-GAP-VERSION-LABEL-001 | UI 不把 internal schema marker 當成產品 generation badge；Process resource defaults顯示`V0.0.0`，GeometryEntity不顯示resource version | Process resource defaults已歸零且geometry version已移除；Step editor仍顯示schema generation badge。`apps/viewer/components/process-step-template-editor/process-step-template-editor.tsx` | integration.platform | Open |
