...
Database internal marker string `"8"`、Process payload wire marker integer `2` 與
GeometryStructure format marker string `"1.0.0"` MUST NOT 混用；三者都不是產品版號。

Version 8移除fixture identity中的release版本並將內建resource metadata歸零，是destructive
schema boundary。任何較舊marker或無markerdatabase都會清空resource tables並由canonical
fixtures重建；未知、非數字或未來marker MUST fail。Repository保存explicit
...
Current internal schema marker 是 `"8"`。Startup policy：

1. Ensure current physical schema can be created/recognized。
2. Read `schema_metadata.databaseSchemaVersion`。
3. Marker缺少或是小於`"8"`的整數時，在同一transaction destructively clear所有resource
   tables；不執行coordinates-to-placements migration。
4. Marker是非數字或大於`"8"`時MUST明確fail且不得刪除資料。
...
Marker already `"8"` 時，startup MUST NOT silently reseed a partially populated database。
