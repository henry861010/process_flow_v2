...
`koz` 用來描述 Via、Circuit 與 Bump density feature 的 keep out zone distance。
`koz` 是非負 finite number，單位依照 structure 的 `unitSystem` 解讀。

Feature 的 `koz` 是給下游 CDB generation 或其他 consumer 使用的 metadata，不會在
process kernel 建立 feature 時預先套用到 `geometry`。因此 Via、Circuit 與 Bump 的
`geometry` 必須保存原本的 feature envelope；任何需要依 `koz` 對 footprint 做
inset/outset、density conversion 或 mesh/CDB 轉換的邏輯，都應在下游 consumer 執行。

Via、Circuit 與 Bump payload 必須明確保存 `koz`。若某個 process step 沒有使用者
輸入或製程參數可描述 keep out zone，建立 feature 時應保存 `koz: 0`。
...
| `koz` | `number` | Via feature 的 keep out zone distance。必須為非負 finite number，不會預先套用到 `geometry`。 |

Via 只作用在持有該 via 的 container scope 內，不會因為幾何座標重疊而自動
向上套用到 parent container，也不會向下套用到 child container。Via 必須保存
`direction` 與 `koz`，後續 process step 不得只由 geometry envelope 的 Z 位置
推論 via 方向，也不得將 keep out zone 視為已經 baked into via geometry。
...
| `koz` | `number` | Circuit feature 的 keep out zone distance。必須為非負 finite number，不會預先套用到 `geometry`。 |

Circuit 只作用在持有該 circuit 的 container scope 內，不會因為幾何座標重疊而自動向上套用到 parent container，也不會向下套用到 child container。Circuit 必須保存 `koz`，且 keep out zone 不代表已經 baked into circuit geometry。
...
| `koz` | `number` | Bump feature 的 keep out zone distance。必須為非負 finite number，不會預先套用到 `geometry`。 |
...
位於哪一側或往哪個方向連接。Bump 也必須保存 `koz`，且 keep out zone 不代表
已經 baked into bump geometry。
...
- Runtime geometry primitive 提供 XY footprint inset/outset 複製能力，供製程 API
  在不直接修改 primitive 內部欄位的情況下產生 body/layer 內縮或外張 envelope。
  此能力不應用來把 Via、Circuit 或 Bump 的 `koz` 預先 baked into feature geometry。
