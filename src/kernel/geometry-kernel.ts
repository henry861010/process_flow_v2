...
type Repository = {
  getById(id: any): Promise<any> | any;
};
...
export class GeometryKernel {
  private _geometryRepository: Repository;
  private _processFlowInstanceRepository: Repository | null;
  private _processFlowTemplateRepository: Repository;
  private _processStepRepository: Repository;
  private _moduleResolver: ProcessStepModuleResolver;
   ...
   */
  async execute(processFlowInstanceOrId: any, options: any = {}): Promise<GeometryKernelExecutionResult> {
    ...
    const stepValueSetsByRefId = new Map<string, any>(
      (processFlowInstance.stepValueSets ?? []).map((valueSet) => [
        valueSet.stepRefId,
        valueSet,
      ]),
    );
    const stepRefs: any[] = processFlowTemplate.stepRefs ?? [];
    const stepRefsById = new Map<string, any>(stepRefs.map((stepRef) => [stepRef.stepRefId, stepRef]));
   ...
   */
  async executePreview(input: any = {}): Promise<any> {
   ...
   */
  async _resolveProcessFlowInstance(processFlowInstanceOrId: any): Promise<any> {
  ...
  async _resolveGeometryInputs({
    ...
    processFlowTemplate,
    stepOutputs,
  }: any): Promise<Map<string, any>> {
    const geometryInputs = new Map<string, any>();
   ...
   */
  async _getByIdOrThrow(repository: Repository, id: any, label: string): Promise<any> {
 ...
 */
function requiredRepository(repository: any, name: string): Repository {
 ...
 */
function topologicalStepRefs(stepRefs: any[], flowEdges: any[]): any[] {
  const stepRefIds = new Set(stepRefs.map((stepRef) => stepRef.stepRefId));
  const incomingCounts = new Map<string, number>(stepRefs.map((stepRef) => [stepRef.stepRefId, 0]));
  const outgoing = new Map<string, string[]>(stepRefs.map((stepRef) => [stepRef.stepRefId, []]));
...
/**
 * Geometry fields are handled separately because they become process state inputs.
 */
...
/**
 * Create an empty process geometry state for steps that do not require geometry input.
 */
