...
export class CadExportOptions {
  ...
  includeFeaturePlaceholders: boolean;
  volumeTolerance: number;
  linearDeflection: number;
  angularDeflection: number;
  opencascade: any;
  suppressOpenCascadeOutput: boolean;
...
export class OpenCascadeConverter {
  private oc: any;
  private options: CadExportOptions;
  ...
  constructor(opencascade: any, options: CadExportOptions | Record<string, any> = {}) {
    if (!opencascade) {
  ...
  async export(payload: any, outputBase: string): Promise<CadExportResult> {
  ...
  convert(payload: any): CadExportResult {
  ...
  _convertContainer(container: any): CadBody[] {
    ...
    const descendantBodies: CadBody[] = [];
  ...
  _bodyToCad(container: any, body: any): CadBody {
    return new CadBody({
      id: body.id,
      sourceIds: [body.id],
  ...
  _resolveSiblingBodies(container: any, bodies: CadBody[]): CadBody[] {
  ...
  _sameMaterialOverlapComponents(bodies: CadBody[]): CadBody[][] {
    const remaining = [...bodies];
    const components: CadBody[][] = [];

    while (remaining.length > 0) {
      const seed = remaining.shift();
      if (seed === undefined) continue;
      const component: CadBody[] = [seed];
      let changed = true;
      while (changed) {
        changed = false;
  ...
  _buildManifest(structure: any, bodies: CadBody[]): any {
    const manifest: any = {
  ...
  _appendFeaturePlaceholders(container: any, manifest: any): void {
    ["vias", "circuits", "bumps"].forEach((featureType) => {
      container[featureType].forEach((feature) => {
        const manifestFeature: Record<string, any> = {
          id: feature.id,
          containerId: container.id,
          containerKey: container.key ?? "",
...
export async function exportCad(payload: any, outputBase: string, options: any = {}) {
  const converter = await OpenCascadeConverter.create(options);
  return converter.export(payload, outputBase);
}

export async function convertCad(payload: any, options: any = {}) {
  const converter = await OpenCascadeConverter.create(options);
  return converter.convert(payload);
}

async function resolveOpenCascade(candidate: any, options: any = {}) {
  ...
  try {
    ...
    try {
      ...
      throw new CadExportError(
        `Unable to load opencascade.js. Run npm install in src first. Original error: ${fallbackMessage || originalMessage}`,
      );
    }
  }
