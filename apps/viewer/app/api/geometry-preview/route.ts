import { GeometryKernel, InMemoryRepository } from "../../../../../src/dist/kernel/index.js";
...
type PreviewExportJob = {
  geometryStructure: any;
  resolve: (value: Buffer) => void;
  reject: (reason?: unknown) => void;
};

let activeGeometryPreviewExports = 0;
const pendingGeometryPreviewExports: PreviewExportJob[] = [];
...
function enqueueGeometryToGlb(geometryStructure: any): Promise<Buffer> {
...
function drainGeometryPreviewExportQueue(): void {
...
async function geometryToGlb(geometryStructure: any): Promise<Buffer> {
...
function runGeometryExportWorker({
  inputPath,
  outputPath,
}: {
  inputPath: string;
  ...
  const workerPath = join(process.cwd(), "dist/scripts/geometry-to-glb-worker.mjs");
  const exporterPath = join(process.cwd(), "../../src/dist/exporters/cad.js");
...
function appendLimited(current: string, chunk: Buffer | Uint8Array | string): string {
...
function normalizePreviewRequest(payload: any): any {
...
function normalizePreviewTarget(payload: any): any {
...
function validatePreviewRequest(request: any): void {
  const { flowTemplate, draftInstance, target } = request;

  const stepTemplateById = new Map<string, any>(
    request.processStepTemplates.map((stepTemplate: any) => [
      stepTemplate.id,
      stepTemplate,
    ]),
  );
  const stepRefsById = new Map<string, any>(
    (flowTemplate.stepRefs ?? []).map((stepRef: any) => [stepRef.stepRefId, stepRef]),
  );
  const valueSetsByStepRef = new Map<string, any>(
    (draftInstance.stepValueSets ?? []).map((valueSet: any) => [
      valueSet.stepRefId,
      valueSet,
    ]),
  );
  const geometryIds = new Set<string>(request.geometries.map((geometry: any) => geometry.id));
  ...
  const edge = (flowTemplate.flowEdges ?? []).find(
    (candidate: any) => candidate.edgeId === target.previewEdgeId,
  );
  if (!edge) {
  ...
  const targetField = targetTemplate?.fieldDefinitions?.find(
    (field: any) => field.id === edge.target?.targetFieldId,
  );
...
function validateFlowEdges(
  flowTemplate: any,
  stepRefsById: Map<string, any>,
  stepTemplateById: Map<string, any>,
): void {
...
function buildStepCompletionChecker({
  ...
  valueSetsByStepRef,
  geometryIds,
}: any): (stepRefId: string, visiting: Set<string>) => { complete: boolean; message: string } {
  const memo = new Map<string, { complete: boolean; message: string }>();
  ...
  return function isComplete(
    ...
    for (const field of stepTemplate.fieldDefinitions ?? []) {
      ...
      if (isGeometryField(field)) {
        ...
        const upstream: { complete: boolean; message: string } = isComplete(
...
function hasStepOutputCycle(flowTemplate: any): boolean {
  const stepIds = new Set<string>((flowTemplate.stepRefs ?? []).map((stepRef: any) => stepRef.stepRefId));
  const adjacency = new Map<string, string[]>([...stepIds].map((stepRefId) => [stepRefId, []]));
  for (const edge of flowTemplate.flowEdges ?? []) {
    if (
      edge.source?.sourceType === "stepOutput" &&
...
function hasInitialGeometryPath(flowTemplate: any, targetStepRefId: string): boolean {
  const queue: string[] = (flowTemplate.flowEdges ?? [])
    .filter((edge: any) => edge.source?.sourceType === "geometryRef")
    .map((edge: any) => edge.target?.stepRefId)
    .filter(Boolean);
  const visited = new Set<string>();
  ...
  while (queue.length > 0) {
    ...
    (flowTemplate.flowEdges ?? [])
      ...
      .forEach((edge: any) => queue.push(edge.target?.stepRefId));
  }

  return false;
}

function isFieldValueComplete(field: any, value: any): boolean {
  ...
  if (field.valueType === "fieldGroupArray") {
    ...
    return items.every((item: any) =>
      childFields.every((childField: any) =>
...
function previewIdForTarget(target: any): string {
  return target.type === "edge"
    ? target.previewEdgeId
    : `step-output-${target.stepRefId}`;
}

function ensureGeometryStructures(geometries: any[]): any[] {
...
function fallbackStructureForGeometry(geometry: any): any {
...
function isGeometryField(field: any): boolean {
  return field?.valueType === "geometryRef" || field?.valueType === "geometry";
}

function isArrayValueType(valueType: any): boolean {
  return (
    valueType === "string[]" ||
    valueType === "integer[]" ||
    valueType === "float[]" ||
...
function fieldValue(fieldValues: any[] | undefined, fieldId: string): any {
...
function expectObject(value: any, label: string): any {
